...
export class Requests {
  ...
  async getPaged(urlComplement: string,
                 page: number,
                 size: number,
                 mensagemErro: string) {

    return this.http.get(`${this.urlBase}${urlComplement}?page=${page}&size=${size}`,
      {headers: this.headers}
    ).toPromise()
      .catch(error => {
        this.carregarMensagemErro(error.error.message, mensagemErro);
  ...
  delete(urlComplent: string, mensagemErro: string) {
    return this.http.delete(`${this.urlBase}${urlComplent}`,
      {headers: this.headers})
      .toPromise()
      .catch(error => {
        console.log(error);
